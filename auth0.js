...
async function auth(settings) {
	...
	try {
		...
		if (settings.authorizationWindowFunction) {
		  ...
		  webRequest.onBeforeRequest({urls: [settings.auth0.redirectUri + '*']}, async ({url}) => {
		...
		ipcMain.handle(settings.calls.data, async () => await axios.get('http://localhost:3000/private', {headers: {'Authorization': `Bearer ${accessToken}`}}).data);
